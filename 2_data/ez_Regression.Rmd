...
dta = import_all %>%
  ...
  filter(year_month > as.yearmon("2013-12")) %>%
  filter(year_month < as.yearmon("2020-03")) %>%
  mutate(USDA_trans = ifelse(year_month > as.yearmon("2016-02") & year_month < as.yearmon("2017-09"), 1, 0),
         USDA_dummy = ifelse(year_month > as.yearmon("2017-08"), 1, 0),
         volume_sum_1m = volume_sum/1000000,
         year = year(year_month),
         month = month(year_month)) %>%
  replace_na(list(refusal_count = 0)) %>%
  drop_na(volume_sum)
...
dta$refusal_count = monthly$total_refusals
...
OLS.1 <- lm(refusal_count ~ USDA_dummy + volume_sum_1m, 
            data = dta)
...
# Create an interaction variable
dta$USDAxVolume = dta$USDA_dummy * dta$volume_sum_1m

OLS.2 <- lm(refusal_count ~ USDA_dummy + volume_sum_1m + USDAxVolume, 
            data = dta)
...
# Add factor(month)
OLS.3 <- lm(refusal_count ~ USDA_dummy + volume_sum_1m + USDAxVolume + factor(month) + factor(country_name),  
            data = dta)
...
OLS.4 <- lm(refusal_count ~ USDA_dummy + USDA_trans + volume_sum_1m + USDAxVolume + factor(month) + factor(country_name), 
            data = dta)
