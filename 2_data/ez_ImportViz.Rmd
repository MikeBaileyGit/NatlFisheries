...
dta_other = dta %>% 
  ...
  group_by(year_month) %>% 
  summarise(country_name = "All Other Countries",
            volume_sum = sum(volume_kg)/1000)
...
dta_VCT$country_name <- tolower(dta_VCT$country_name)
substr(dta_VCT$country_name, 1, 1) <- toupper(substr(dta_VCT$country_name, 1, 1))
...
dta_country = dta_VCT %>% 
  bind_rows(dta_other) %>%
  arrange(year_month, country_name)
...
dta_country$country_name <- factor(dta_country$country_name, levels = c("Vietnam", "China", "Thailand","All Other Countries"))
bluePalette <- c("#041E42", "#003D62", "#6A98BA", "#CAE2E0")
  
ggplot(dta_country,aes(year_month,volume_sum)) + 
  geom_path(aes(color=country_name), size=0.75) + 
  geom_vline(xintercept = as.yearmon(c('Mar 2016','Nov 2019')), 
             color='#862938', linetype='dashed', size=0.75) + 
  annotate('text', x = as.yearmon('Mar 2016'), y = 14500, 
           label = ' Mar 2016:\n Inspection responsibility\n transferred from FDA to USDA', 
           size = 3, color = '#862938', hjust = 0) + 
...
dta_map[1,2] = dta_map[1,2]/350
dta_map[2,2] = dta_map[2,2]/25
...
ggsave('output/figures/import/catfish_import_map.png',dpi=300,bg="transparent")
...
dta2 <- read.csv("output/build/monthly_2002.csv")
dta2 = subset(dta2, select = -c(X))
dta2$year_month <- as.yearmon(paste(dta2$year, dta$month), "%Y %B")
dta2 = as_tibble(dta2)
glimpse(dta2)
...
dta2_vietnam_year = dta2_vietnam %>% 
  mutate(dategroup = floor_date(as.Date(year_month), "1 year")) %>%
  group_by(dategroup) %>%
  summarise(year_volume = sum(volume_sum)/1000,
            year_value = sum(value_sum)/1000)
...
ggplot(dta2_vietnam_year,aes(dategroup, year_volume)) + 
  ...
  #          label = ' Nov 2019:\n USDA granted\n equivalence to\n Vietnam',
  #          size = 3, color = 'steelblue4', hjust = 0) +
  # labs(title = 'Volume of Catfish Imported from Vietnam by Year from 2002 to 2020', 
  #      x = 'Date', y = 'Volume of Catfish Imported from Vietnam (million kg)') + 
  labs(title = 'Volume of Catfish Imported from Vietnam by Year from 2002 to 2020', 
       x = '', y = '') + 
  scale_x_date(date_breaks = "1 year", date_labels = "%Y") +
...
ggsave('output/figures/import/catfish_vietnam_2002to2020.png',dpi=300)
